...
function signatureValid(body:string,request:Request,message:SoulMeshMessage){
 ...
 if(hmacSecret&&nonce&&hmac)return verifySoulMeshMessage(message,hmacSecret,nonce,hmac);
 const secret=process.env.SOUL_MESH_SIGNING_SECRET;
 if(!secret)return process.env.NODE_ENV!=='production';
 const provided=request.headers.get('x-soul-signature');if(!provided)return false;
...
function response(message:SoulMeshMessage,payload:unknown,kind:'response'|'error'='response',traceId=crypto.randomUUID()){return {protocol:SOUL_MESH_PROTOCOL,id:crypto.randomUUID(),correlationId:message.correlationId,source:NUCLEUS_ID,target:message.source,kind,capability:message.capability,payload,timestamp:Date.now(),meta:{runtime:'nextjs-ai-chatbot',transport:'http-json',encoding:'json',version:SOUL_MESH_PROTOCOL,nonce:crypto.randomUUID(),traceId}}}
