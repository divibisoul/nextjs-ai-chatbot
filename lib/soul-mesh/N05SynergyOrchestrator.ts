...
export function composeN05WithN06(prompt: string) {
  return executeN05Synergy([
    { nucleus: 'N06', capability: 'support.ai-pilot', payload: { task: prompt } },
    { nucleus: 'N02', capability: 'inference.reason', payload: { prompt: 'Reason over the N06 plan and identify the best solution.' } },
...
export function composeN05WithN04(prompt: string) {
  return executeN05Synergy([
    ...
    { nucleus: 'N06', capability: 'support.ai-pilot', payload: { task: 'Review and plan the next action for the artifact.' } },
  ]);
}
