...
export async function handleMeshMessage(message: SoulMeshMessage, handlers: Record<string, (payload: unknown) => Promise<unknown> | unknown>) {
  ...
  try {
    ...
    return { ...message, kind: message.capability === 'mesh.handshake' ? ('ack' as const) : ('response' as const), source: NUCLEUS_ID, target: message.source, proof: 'EXECUTED' as const, payload };
  } catch (error) {
