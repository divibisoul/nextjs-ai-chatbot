...
function payloadToRequest(payload: unknown, system: string): SoulInferenceRequest { if (typeof payload === 'string') return { prompt: payload, system, model: 'chat-model' }; if (!payload || typeof payload !== 'object') throw new TypeError('N05_INFERENCE_PAYLOAD_REQUIRED'); return { ...(payload as Partial<SoulInferenceRequest>), system } as SoulInferenceRequest; }
...
export function createN05CapabilityGateway() {
  ...
  const inferenceAgent: N05Agent = { id:'N05-inference-agent', name:'N05 Inference Agent', capabilities:Object.keys(systems), execute: async (request) => { const system = systems[request.capability]; const input = payloadToRequest(request.payload, system); if (request.capability.startsWith('conversation.')) return n05InferencePool.run(input, request.source === 'N01' ? 100 : 50); const cached = cache.get(input); if (cached !== undefined) return cached; const result = await n05InferencePool.run(input, request.source === 'N01' ? 100 : 50); cache.set(input, result); return result; } };
  agents.register(inferenceAgent);
  for (const capability of Object.keys(systems)) gateway.register(capability, (request) => agents.execute(request), N05_OWNERSHIP[capability.startsWith('conversation.') ? 'conversation.' : 'inference.']);
  return gateway;
}
