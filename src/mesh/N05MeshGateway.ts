...
import { callNucleus } from '../../lib/soul-mesh/SoulMeshAdapter';
...
export class N05MeshGateway {
  ...
  async execute(request: MeshRequest): Promise<MeshResponse> {
    ...
    // The other nuclei have evolved their ownership contracts. N05 must delegate
    // non-N05 capabilities instead of pretending that a local handler exists.
    if (rule.owner !== 'N05') {
      const candidates = [rule.owner, ...(rule.fallback ?? [])].filter((value, index, all) => all.indexOf(value) === index && value !== 'N05');
      let lastError: unknown;
      for (const target of candidates) {
        try {
          const result = await callNucleus(target, request.capability, request.payload, { timeoutMs: 30000 });
