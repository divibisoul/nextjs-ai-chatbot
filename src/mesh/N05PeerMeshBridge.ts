...
export class N05PeerMeshBridge {
 ...
 async request(peer:NucleusId,capability:string,payload:unknown,correlationId=randomUUID(),traceId=randomUUID()){
  ...
  const hmac=hmacSecret?signSoulMeshMessage(envelope as never,hmacSecret,nonce):undefined;
  let last:unknown;
  for(let attempt=0;attempt<=this.retries;attempt++){
   ...
   try{const headers:Record<string,string>={'content-type':'application/json',accept:'application/json','x-soul-nucleus':'N05','x-soul-target':peer,'x-correlation-id':correlationId,'x-soul-trace-id':traceId,traceparent:`00-${traceId.replaceAll('-','').slice(0,32).padEnd(32,'0')}-${envelope.id.replaceAll('-','').slice(0,16).padEnd(16,'0')}-01`};if(hmacSecret&&hmac){headers['x-soul-mesh-nonce']=nonce;headers['x-soul-mesh-hmac']=hmac;}const response=await fetch(this.endpoint(peer),{method:'POST',headers,body:JSON.stringify(envelope),cache:'no-store',signal:controller.signal});const text=await response.text();let body:unknown=text;try{body=text?JSON.parse(text):null;}catch{}if(!response.ok)throw new Error(`MESH_HTTP_${response.status}`);state.healthy=true;state.failures=0;state.latencyMs=Date.now()-started;return{peer,payload:body,status:response.status,latencyMs:state.latencyMs,attempt};}
